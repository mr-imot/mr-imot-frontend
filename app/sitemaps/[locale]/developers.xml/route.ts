...
/**
 * Proxy locale sitemap (developers) from backend.
...
export async function GET(
  ...
  try {
    ...
    const backendUrl = `${baseApiUrl}/api/v1/sitemaps/${encodeURIComponent(locale)}/developers.xml`
    ...
    if (!response.ok) {
      console.error('[sitemaps] Backend developers.xml non-OK:', response.status, locale)
    ...
    console.error('[sitemaps] Error proxying developers.xml:', error)
    return new Response(null, {
      status: 502,
      headers: { 'Cache-Control': 'no-store' },
    })
