...
export default function TestAPIPage() {
  ...
  const testRealAPI = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? '' : 'https://api.mrimot.com')}/api/v1/projects/`)
  ...
  const testMockAPI = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || (process.env.NODE_ENV === 'development' ? '' : 'https://api.mrimot.com')}/api/v1/mock-projects/`)
