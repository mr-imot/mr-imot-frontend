    ...
    try {
      ...
      if (response.ok) {
        ...
        this.storeNotificationHistory({
          id: `notif_${Date.now()}`,
          to: developer.email,
          subject: '🎉 Вашата заявка е одобрена!',
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.storeNotificationHistory({
          id: `notif_${Date.now()}`,
          to: developer.email,
          subject: 'Актуализация на вашата заявка',
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.storeNotificationHistory({
          ...
          subject: `Нова заявка: ${developer.company_name}`,
    ...
    return {
      developer_verified: {
        ...
        htmlContent: `
          ...
          <p>Радваме се да ви информираме, че вашата заявка за <strong>{{companyName}}</strong> е одобрена!</p>
        ...
        textContent: `
          ...
          Радваме се да ви информираме, че вашата заявка за {{companyName}} е одобрена!
      ...
      developer_rejected: {
        subject: 'Актуализация на вашата заявка',
        htmlContent: `
          <h2>Актуализация на заявката</h2>
          <p>Уважаеми {{contactPerson}},</p>
      ...
      new_application: {
        subject: 'Нова заявка: {{companyName}}',
        htmlContent: `
          <h2>Получена нова заявка</h2>
          <p>Нов инвеститор е подал заявка:</p>
          <ul>
